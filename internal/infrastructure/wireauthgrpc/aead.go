...
// encryptRecord seals plaintext into a single AEAD record BODY (the part
// that follows the 4-byte record_len prefix written separately by
// secureConn — see protocol.go):
//
//	seq(8, BE) || nonce(12) || ciphertext+tag
//
// AAD is the 8-byte big-endian seq (not the nonce, not record_len). seq
// must be unique per (key, direction) — secureConn enforces this via a
// monotonic per-connection, per-direction counter; callers of this
// function directly (e.g. tests) are responsible for the same invariant.
...
// decryptRecord opens a record produced by encryptRecord. It returns the
// seq embedded in the record so callers can check it against their
// expected receive counter (secureConn does this to detect reordering /
// replay / drops).
